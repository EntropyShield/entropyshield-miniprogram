...
Page({
  ...
  onShow() {
    ...
    try {
      if (wx.__tryFinishRewardOnReportNav) {
        wx.__tryFinishRewardOnReportNav('/pkgChallenge/campReport/index');
      }
    } catch (e) {}
  ...
  buildReport() {
    ...
    let levelTag = '记录起步';
    let brief = '不妨先坚持连续记录 7 天，再来看账户和心态的变化。';
    ...
    if (avgScore < 60) {
      suggestions.push('目前风控执行还有明显漏网之鱼，不妨先把「单笔最大亏损」「单日最大回撤」这两条铁律写清楚并严格遵守。');
    } else {
  ...
  goMembership() {
    wx.navigateTo({
      url: '/pages/pay/index?type=steady&from=campReport'
    });
  }
});
