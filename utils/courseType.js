...
const TYPE_META = {
  PUBLIC: {
    ...
    pathNextStep:
      '课后推荐：安装熵盾风控计算器，预约一次体验课或账户体检，把理念落到自己的账户上。'
  },
  ...
  EXPERIENCE: {
    ...
    pathNextStep:
      '课后推荐：正式用风控计算器跑一套自己的完整方案，然后进入 7 天风控训练营做行为固化。'
  },
  ...
  CAMP: {
    ...
    pathNextStep:
      '课后推荐：进入风控系统课/进阶课，把训练营成果升级为可长期复用的风控体系。'
  },
  ...
  RISK: {
    ...
    pathNextStep:
      '课后推荐：结合训练营，把这套风控系统写进自己的“交易前后必做动作清单”里。'
  },
  ...
  SALON: {
    ...
    pathNextStep:
      '线下推荐：根据账户体检结果，决定是否进入控局者系统课/进阶营，并与顾问讨论合伙人路径。'
  },
  ...
  CONTROLLER: {
    ...
    pathStageDesc:
      '面向希望长期在市场活下去并实现资金稳健管控的用户，完整输出熵盾守护者的风控与纪律体系。',
    pathNextStep:
      '课后推荐：配合风控工具 + 训练营 + 线下陪跑，进入长期风控体系，并视情况加入合伙人机制。'
  },
  ...
  UNKNOWN: {
    ...
    pathStageDesc:
      '课程类型暂未归类，但仍然属于控局路径中的一环，可以作为你风控旅程的一部分。',
    pathNextStep:
      '可与你的熵盾顾问确认这门课在控局路径中的具体位置，再决定下一步应该衔接哪一类课。'
  }
};
