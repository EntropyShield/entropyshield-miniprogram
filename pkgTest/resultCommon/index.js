...
// [V2.0-C2] 接入统一分享工具：让每次测试结果分享都带上邀请码与定制文案
...
Page({
  ...
  onLoad(options) {
    ...
    // [V2.0-C2] 预拉取本人邀请码，供分享时同步拼进路径（异步，不阻塞渲染）
    share.refreshInviteCode();
  },
  ...
  buildProfile(type, score, levelParam) {
    ...
    switch (type) {
      ...
      case 'camp': {
        ...
        if (level === 'low') {
          base.title = '本轮训练执行度偏低，不妨重新走一遍 7 天任务';
          base.shortDesc =
            '你的记录较为零散，很多关键任务没有完成，不妨放慢节奏，用一轮时间专门练习「记录 + 复盘」。';
        ...
        base.planHint = '不妨用下一轮 7 天训练，把“执行力”进一步固化下来：';
        base.planItems = [
          {
            day: 'D1',
            title: '固定一个记录模板',
      ...
      case 'market':
        ...
        if (level === 'low') {
          ...
          base.shortDesc =
            '你对风险有感知，但执行上容易受热点影响，降低整体仓位有助于控制风险。';
          base.badges = ['容易追高', '对回调容忍度低', '方向感尚可'];
        } else {
          base.title = '当前为高风险区，请优先考虑防守而非进攻';
          base.shortDesc =
            '你的风险判断与实际市场波动存在偏差，先减仓再重新评估更稳妥。';
      ...
      case 'danger':
        ...
        if (level === 'low') {
          ...
          base.title = '危险等级偏高，立即降低仓位并进入修复模式可控制风险';
          base.shortDesc =
  ...
  goCamp() {
    wx.navigateTo({
      url: '/pkgChallenge/campIntro/index'
    });
  },
  ...
  // ====== [V2.0-C4] 分享卡：生成 → 预览 → 保存相册 ======
  // 小程序码复用既有 /api/fission/qrcode 接口（fissionTask 页已在用），
  // 落地页指向"测试入口页"而非结果页 —— 新用户扫码是自己测，不是看别人的结果。
  onMakeCard() {
    const url = this._buildQrcodeUrl();
    this.setData({ qrcodeUrl: url, cardVisible: true });
    this._log('RESULT_CARD_OPEN', { type: this.data.type, score: this.data.score });
  },
  ...
  onCardSaved() {
    this._log('RESULT_CARD_SAVE', { type: this.data.type, score: this.data.score });
  },

  onCardError(e) {
    this._log('RESULT_CARD_FAIL', { type: this.data.type, reason: e.detail && e.detail.err });
  },

  _buildQrcodeUrl() {
    const base = String((CONFIG && CONFIG.API_BASE) || '').replace(/\/+$/, '');
    const code = share.getInviteCode();
    if (!base || !code) return ''; // 拿不到就画占位框，不阻塞出图
    ...
    const raw = share.TEST_PAGES[this.data.type] || share.TEST_PAGES.danger;
    const page = String(raw).replace(/^\//, ''); // 接口要求不带前导斜杠
  ...
  // 埋点：只记行为，不含任何持仓/价格信息（12 号合规报告）
  _log(evt, extra) {
    try {
      const funnel = require('../../utils/funnel.js');
      funnel.log(evt, extra || {});
    } catch (e) {}
  },

  // ====== [V2.0-C2] 分享：结果页 → 测试入口页（带邀请码）======
  // 落地页刻意指向"测试入口"而非"结果页"：新用户点开后是自己测一遍，
  // 而不是看别人的结果 —— 这样才能形成"测 → 晒 → 别人测 → 再晒"的循环。
  onShareAppMessage() {
    const d = this.data;
    const cfg = share.buildTestShare(d.type, d.score, d.level, d.profile && d.profile.levelText);
    // 埋点：记录一次分享动作（合规：只记行为，不含任何持仓/价格信息）
    try {
      const funnel = require('../../utils/funnel.js');
      funnel.log('RESULT_SHARE', { type: d.type, score: d.score });
    } catch (e) {}
    return cfg;
  },
  ...
  onShareTimeline() {
    const d = this.data;
    return share.buildTestShare(d.type, d.score, d.level, d.profile && d.profile.levelText);
  }
});
