...
Page({
  ...
  onCourseTap(e) {
    ...
    if (!id) return;
    wx.navigateTo({ url: `/pkgService/course/detail?id=${id}&from=courses` });
  },

  onSalonBooking(e) {
    const id = e.currentTarget.dataset.id;
    wx.navigateTo({ url: `/pkgService/visitBooking/index?from=course&courseId=${id || ''}` });
  },

  goCourseProgress(e) {
    const focusCourseId = (e && e.currentTarget && e.currentTarget.dataset && e.currentTarget.dataset.id) || '';
    wx.navigateTo({
      url: `/pkgService/course/progress?from=courses${focusCourseId ? `&focusCourseId=${focusCourseId}` : ''}`
    });
  },
