...
Page({
  ...
  onSalonBooking() {
    ...
    wx.navigateTo({
      url: `/pkgService/visitBooking/index?from=courseDetail&courseId=${course.id}`
    });
  },
  ...
  goProgress() {
    ...
    wx.navigateTo({
      url: `/pkgService/course/progress?from=courseDetail${courseId ? `&focusCourseId=${courseId}` : ''}`
    });
  },
  ...
  onShareAppMessage() {
    ...
    const path =
      `/pkgService/course/detail?id=${courseId || ''}` +
      `&from=share` +
